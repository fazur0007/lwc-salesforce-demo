({
    ...
    goOnApplianceRegisterSummary : function(component, event, helper) { 
        ...
        else if (component.get("v.newAccount.PersonBirthdate") === ''){
      ...
      dropVali.set("v._errorMessage", "Date cannot be future date");
        ...
        if(selectedSubProductValue !='' && selectedModelValue !='' && placeOfPurchaseValue !='' && blank==0){
            helper.helperMethodForProductRegister(component, event, helper);
        }
    }, 
    ...
    goRegistrationSummary : function(component, event,helper) {
        ...
        if(Phonevalue == undefined || Phonevalue == '' ){
            var Mandatorylabel = $A.get("$Label.c.is_Mandatory");
            var PhoneLabel = $A.get("$Label.c.PhoneV3");
            let newPhoneLabel = PhoneLabel.replace('*','');
            var StringValue = newPhoneLabel +' '+ Mandatorylabel;
            helper.customValidation(component, event, PhoneId,StringValue);
            //return;
        }
        if(AddressLinevalue ==='' || AddressLinevalue === undefined){
            var Mandatorylabel = $A.get("$Label.c.is_Mandatory");
            var AddressLineLabel = $A.get("$Label.c.AddressV1");
            let newAddressLineLabel = AddressLineLabel.replace('*','');
            var StringValue = newAddressLineLabel +' '+ Mandatorylabel;
        ...
        if(BillingCityValue ==='' || BillingCityValue === undefined){
            var Mandatorylabel = $A.get("$Label.c.is_Mandatory");
            var BillingCityLabel = $A.get("$Label.c.CityV1");
            let newBillingCityLabel = BillingCityLabel.replace('*','');
            var StringValue = newBillingCityLabel +' '+ Mandatorylabel;
            helper.customValidation(component, event, BillingCityId,StringValue);
            // return;
        }
        if(PostalCodeValue ==='' || PostalCodeValue === undefined){
            var Mandatorylabel = $A.get("$Label.c.is_Mandatory");
            var PostalCodeLabel = $A.get("$Label.c.postalCodeV1");
            let newPostalCodeLabel = PostalCodeLabel.replace('*','');
            var StringValue = newPostalCodeLabel +' '+ Mandatorylabel;
        ...
        console.log('BillingCityValue>>>>>',BillingCityValue);
    ...
    showRecType : function(component, event, helper) {
        ...
        //console.log('The selected Record Type is: ' +component.get("v.newAccount.BillingCountry"));
        console.log('The selected billingCountryValue: ' ,component.get("v.billingCountryValue"));
