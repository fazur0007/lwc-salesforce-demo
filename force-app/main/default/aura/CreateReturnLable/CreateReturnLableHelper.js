({
    doInit : function(component, event, helper) {
        helper.apexUtil(component,helper,'createReturnLabel',{ recordId : component.get("v.recordId")})
        .then(function(result){
            window.objectConfig = result;
            console.log(JSON.stringify(objectConfig));
            if(result === "Success"){
                helper.errorUtil(component,'Case Record','record has been updated successfully','success');
                helper.navigationUtil(component,"view","Case",component.get("v.recordId"));
                component.set("v.showLoader",false);
            }else{
                helper.errorUtil(component,'Case',result,'warning');
                helper.navigationUtil(component,"view","Case",component.get("v.recordId"));
                component.set("v.showLoader",false);
            }
        })
